<x-app-layout>
    ...
    <div style="max-width: 900px; margin: 0 auto; padding: 20px;">
        <!-- Back Button -->
        <a href="{{ route('brand-users.index') }}" style="display: inline-flex; align-items: center; color: #848177; text-decoration: none; margin-bottom: 25px; font-size: 14px; font-weight: 500; transition: all 0.3s ease;">
        ...
        <div style="background: white; border-radius: 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06); overflow: hidden;">
            ...
            <div style="background: linear-gradient(180deg, #848177 0%, #000000 100%); padding: 30px; color: white;">
                <div style="display: flex; align-items: center; gap: 15px;">
                    ...
                    <div>
                        <h1 style="font-size: 28px; font-weight: 700; margin: 0 0 5px 0;">Edit User</h1>
                        <p style="font-size: 14px; opacity: 0.9; margin: 0;">Update brand user details</p>
                    </div>
                </div>
            </div>

            <!-- Form Section -->
            <form method="POST" action="{{ route('brand-users.update', $brandUser) }}" style="padding: 35px;">
                @csrf
                @method('PUT')
                    ...
                    <div style="position: relative;">
                        <select id="brand_ids" 
                            ...
                            @foreach($brands as $brand)
                                <option value="{{ $brand->id }}" {{ in_array($brand->id, old('brand_ids', $userBrandIds ?? [])) ? 'selected' : '' }}>{{ $brand->name }}</option>
                            @endforeach
                        </select>
                ...
                <div style="margin-bottom: 30px;">
                    ...
                    <div style="position: relative;">
                        <input type="email" 
                               id="email" 
                               name="email" 
                               value="{{ old('email', $brandUser->email) }}" 
                               required 
                ...
                <!-- Language Selection -->
                <div style="margin-bottom: 30px;">
                    <label for="language_id" style="display: block; color: #1f2937; font-size: 14px; font-weight: 600; margin-bottom: 10px;">
                        <span style="display: flex; align-items: center; gap: 8px;">
                            <svg style="width: 18px; height: 18px; color: #848177;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div style="position: relative;">
                        <select id="language_id" 
                            ...
                            @foreach($languages as $language)
                                <option value="{{ $language->id }}" {{ old('language_id', $brandUser->language_id) == $language->id ? 'selected' : '' }}>{{ $language->name }}</option>
                            @endforeach
                        </select>
                        <svg style="position: absolute; left: 16px; top: 50%; transform: translateY(-50%); width: 20px; height: 20px; color: #9ca3af; pointer-events: none;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129"></path>
                ...
                <!-- Role Selection -->
                <div style="margin-bottom: 30px;">
                    <label for="role_id" style="display: block; color: #1f2937; font-size: 14px; font-weight: 600; margin-bottom: 10px;">
                        <span style="display: flex; align-items: center; gap: 8px;">
                            <svg style="width: 18px; height: 18px; color: #848177;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                            </svg>
                    ...
                    <div style="position: relative;">
                        <select id="role_id" 
                            ...
                            {{-- @foreach($roles as $role)
                                <option value="{{ $role->id }}" {{ old('role_id', $brandUser->roles->first()?->id) == $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                            @endforeach --}}
                            
                                @foreach($roles as $role)
                                    <option value="{{ $role->id }}"
                                        {{ in_array($role->id, old('role_id', $userRoleIds)) ? 'selected' : '' }}>
                                        {{ $role->name }}
                                    </option>
                                @endforeach
