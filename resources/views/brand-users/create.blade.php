        ...
        <div style="background: white; border-radius: 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06); overflow: hidden;">
            ...
            <div style="background: linear-gradient(180deg, #848177 0%, #000000 100%); padding: 30px; color: white;">
                <div style="display: flex; align-items: center; gap: 15px;">
                    ...
                    <div>
                        <h1 style="font-size: 28px; font-weight: 700; margin: 0 0 5px 0;">Create New User</h1>
                        <p style="font-size: 14px; opacity: 0.9; margin: 0;">Define a new brand user account</p>
                    </div>
                </div>
            </div>

            <!-- Form Section -->
            <form method="POST" action="{{ route('brand-users.store') }}" style="padding: 35px;">
                @csrf
                    ...
                    <div style="position: relative;">
                        <select id="brand_ids" 
                            ...
                            @foreach($brands as $brand)
                                <option value="{{ $brand->id }}" {{ in_array($brand->id, old('brand_ids', [])) ? 'selected' : '' }}>{{ $brand->name }}</option>
                            @endforeach
                        </select>
