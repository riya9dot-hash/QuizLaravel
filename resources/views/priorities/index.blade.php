    ...
    <div style="max-width: 1200px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: white; border-radius: 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06); padding: 25px; margin-bottom: 25px;">
            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                <div>
                    <h1 style="font-size: 24px; font-weight: 700; color: #1f2937; margin: 0 0 5px 0;">My Created Priorities</h1>
                    <p style="font-size: 14px; color: #6b7280; margin: 0;">Manage and organize your priority settings</p>
                </div>
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background: linear-gradient(180deg, #848177 0%, #000000 100%);">
                            ...
                            <th style="padding: 16px 20px; text-align: left; color: white; font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">Priority Name</th>
                            <th style="padding: 16px 20px; text-align: left; color: white; font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">Point</th>
                        ...
                        @forelse($priorities as $priority)
                            <tr class="table-row" style="border-bottom: 1px solid #f3f4f6; transition: all 0.2s ease;">
                                <td style="padding: 18px 20px;">
                                    <input type="checkbox" class="priority-checkbox" data-priority-id="{{ $priority->id }}" style="width: 18px; height: 18px; cursor: pointer; accent-color: #848177;">
                                </td>
                                <td style="padding: 18px 20px; color: #6b7280; font-size: 14px; font-weight: 500;">#{{ $priority->id }}</td>
                                <td style="padding: 18px 20px;">
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        ...
                                        <div>
                                            <div style="color: #1f2937; font-size: 15px; font-weight: 600; margin-bottom: 2px;">{{ $priority->name }}</div>
                                            <div style="color: #9ca3af; font-size: 12px;">Priority</div>
                                        </div>
                                    </div>
                                </td>
                                <td style="padding: 18px 20px;">
                                    <span style="background: #dbeafe; color: #1e40af; padding: 4px 12px; border-radius: 12px; font-size: 12px; font-weight: 500;">{{ $priority->point }}</span>
                                </td>
                                ...
                                <td style="padding: 18px 20px;">
                                    <div style="display: flex; align-items: center; justify-content: center; gap: 8px;">
                                        <a href="{{ route('priority.edit', $priority) }}" style="display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; background: #f0f4ff; color: #848177; border-radius: 8px; text-decoration: none; transition: all 0.2s ease;" onmouseover="this.style.background='#e0e7ff'; this.style.transform='scale(1.1)';" onmouseout="this.style.background='#f0f4ff'; this.style.transform='scale(1)';" title="Edit Priority">
                                            <svg style="width: 18px; height: 18px;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                            </svg>
                                        </a>
                                        <form action="{{ route('priority.destroy', $priority) }}" method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this priority? This action cannot be undone.');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" style="display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; background: #fee2e2; color: #ef4444; border: none; border-radius: 8px; cursor: pointer; transition: all 0.2s ease;" onmouseover="this.style.background='#fecaca'; this.style.transform='scale(1.1)';" onmouseout="this.style.background='#fee2e2'; this.style.transform='scale(1)';" title="Delete Priority">
                                                <svg style="width: 18px; height: 18px;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                                </svg>
                                            </button>
                            ...
                            <tr>
                                <td colspan="6" style="padding: 60px 20px; text-align: center;">
                                    <div style="max-width: 400px; margin: 0 auto;">
                                        ...
                                        <h3 style="font-size: 18px; font-weight: 600; color: #374151; margin-bottom: 8px;">No priorities found</h3>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateDeleteButton() {
                const checkedBoxes = document.querySelectorAll('.priority-checkbox:checked');
            ...
            if (deleteSelectedBtn) {
                deleteSelectedBtn.addEventListener('click', function(e) {
                    ...
                    if (checkedBoxes.length === 0) {
                        alert('Please select at least one priority to delete.');
                        return false;
                    }

                    // Collect selected priority IDs
                    const priorityIds = Array.from(checkedBoxes).map(cb => cb.getAttribute('data-priority-id'));
                    
                    const count = priorityIds.length;
                    if (confirm(`Are you sure you want to delete ${count} priority(ies)?`)) {
                        // Create a dynamic form and submit it
                        const form = document.createElement('form');
                        form.method = 'POST';
                        form.action = '{{ route("priority.bulk-delete") }}';
