        ...
        <div style="background: white; border-radius: 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06); overflow: hidden;">
            ...
            <div style="background: linear-gradient(180deg, #848177 0%, #000000 100%); padding: 30px; color: white;">
                <div style="display: flex; align-items: center; gap: 15px;">
                    ...
                    <div>
                        <h1 style="font-size: 28px; font-weight: 700; margin: 0 0 5px 0;">Edit Quiz</h1>
                        <p style="font-size: 14px; opacity: 0.9; margin: 0;">Update quiz question and answer details</p>
                    </div>
                </div>
            </div>

            <!-- Form Section -->
            <form method="POST" action="{{ route('quiz.update', $quiz) }}" style="padding: 35px;">
                @csrf
                @method('PUT')
                ...
                <div style="margin-bottom: 30px;">
                    ...
                    <div style="position: relative;">
                        <textarea id="question" 
                                  ...
                                  onfocus="this.style.borderColor='#848177'; this.style.background='white'; this.style.boxShadow='0 0 0 3px rgba(102, 126, 234, 0.1)';"
                                  onblur="this.style.borderColor='#e5e7eb'; this.style.background='#f9fafb'; this.style.boxShadow='none';">{{ old('question', $quiz->question) }}</textarea>
                ...
                <div style="margin-bottom: 30px;">
                    ...
                    <div style="display: flex; gap: 12px;">
                        <label class="answer-option" style="display: flex; align-items: center; cursor: pointer; padding: 14px 20px; background: white; border: 2px solid #e5e7eb; border-radius: 10px; transition: all 0.2s ease; flex: 1;" onmouseover="if(!this.querySelector('input').checked) this.style.borderColor='#d1d5db'; this.style.background='#f9fafb';" onmouseout="if(!this.querySelector('input').checked) this.style.borderColor='#e5e7eb'; this.style.background='white';">
                            <input type="radio" name="answer" value="true" id="answer_true" {{ old('answer', $quiz->answer) == 'true' ? 'checked' : '' }} required style="width: 18px; height: 18px; margin-right: 12px; cursor: pointer; accent-color: #848177; flex-shrink: 0;" onchange="updateAnswerStyles()">
                            <span style="font-size: 15px; font-weight: 500; color: #1f2937; user-select: none;">True</span>
                        </label>
                        <label class="answer-option" style="display: flex; align-items: center; cursor: pointer; padding: 14px 20px; background: white; border: 2px solid #e5e7eb; border-radius: 10px; transition: all 0.2s ease; flex: 1;" onmouseover="if(!this.querySelector('input').checked) this.style.borderColor='#d1d5db'; this.style.background='#f9fafb';" onmouseout="if(!this.querySelector('input').checked) this.style.borderColor='#e5e7eb'; this.style.background='white';">
                            <input type="radio" name="answer" value="false" id="answer_false" {{ old('answer', $quiz->answer) == 'false' ? 'checked' : '' }} required style="width: 18px; height: 18px; margin-right: 12px; cursor: pointer; accent-color: #848177; flex-shrink: 0;" onchange="updateAnswerStyles()">
                            <span style="font-size: 15px; font-weight: 500; color: #1f2937; user-select: none;">False</span>
                        </label>
                        <label class="answer-option" style="display: flex; align-items: center; cursor: pointer; padding: 14px 20px; background: white; border: 2px solid #e5e7eb; border-radius: 10px; transition: all 0.2s ease; flex: 1;" onmouseover="if(!this.querySelector('input').checked) this.style.borderColor='#d1d5db'; this.style.background='#f9fafb';" onmouseout="if(!this.querySelector('input').checked) this.style.borderColor='#e5e7eb'; this.style.background='white';">
                            <input type="radio" name="answer" value="both" id="answer_both" {{ old('answer', $quiz->answer) == 'both' ? 'checked' : '' }} required style="width: 18px; height: 18px; margin-right: 12px; cursor: pointer; accent-color: #848177; flex-shrink: 0;" onchange="updateAnswerStyles()">
                    ...
                    <div style="position: relative;">
                        <select id="priority_id" 
                            ...
                            @foreach($priorities as $priority)
                                <option value="{{ $priority->id }}" {{ old('priority_id', $quiz->priority_id) == $priority->id ? 'selected' : '' }}>{{ $priority->name }} ({{ $priority->point }} points)</option>
                            @endforeach
                        </select>
                        <svg style="position: absolute; left: 16px; top: 50%; transform: translateY(-50%); width: 20px; height: 20px; color: #9ca3af; pointer-events: none;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path>
